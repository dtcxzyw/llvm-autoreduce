...
def reprocess_issue(issue):
    ...
    extract_prompt = (
        ...
        "This is CRITICAL: O2 bugs need O2 IRGen-level IR, e.g.:\n"
        "   clang -x c -O2 -Xclang -disable-llvm-passes -S -emit-llvm source.c -o reproducer.ll\n"
        "   This disables LLVM passes so clang emits O2 IRGen-level IR without running "
        "any optimization passes. Use -O1/-O2/-O3 to match the issue's optimization level.\n"
